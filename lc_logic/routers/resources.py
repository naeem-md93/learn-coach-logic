...
from lc_logic.services.pdf import mask_url
...
async def extract_title_endpoint(payload: ExtractTitleRequest) -> ExtractTitleResponse:
    ...
    """
    # file_url carries a Django-issued signed token as a query param — never
    # log it in full, only scheme+host+path (see mask_url).
    logger.info("POST /extract-title file_url=%s", mask_url(str(payload.file_url)))

    try:
        pdf_bytes = await download_pdf(str(payload.file_url))
        leading_text = extract_leading_text(pdf_bytes)
    except PdfFetchError as exc:
        logger.warning("extract-title failed while fetching/parsing the PDF: %s", exc)
        raise HTTPException(status_code=422, detail=str(exc)) from exc

    try:
        title = await extract_title(leading_text)
    except TitleExtractionError as exc:
        logger.warning("extract-title failed during Gemini title extraction: %s", exc)
